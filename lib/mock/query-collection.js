...
/*
* Get Mock Collection from schema.default or schema.example
* If there is no mock data, then it fallback to origin implementation
* @param   {Request}   req
* @param   {Response}  res
* @param   {function}  next
* @param   {DataStore} dataStore
*/
function getMockCollection(req, res, next, dataStore) {
  if (res.swagger.examples) {
    util.debug('Using mock data from examples');
    var examples = res.swagger.examples;
    var resources = examples[Object.keys(examples)[0]];
    res.body = filter(resources, req);

    util.debug('isArray', Array.isArray(res.body));
    if (!Array.isArray(res.body)) {
      res.swagger.isWrapped = false;
    }

    return next();
  }

  util.debug('Using mock data from dataStore');
  return queryCollection(req, res, next, dataStore);
}
